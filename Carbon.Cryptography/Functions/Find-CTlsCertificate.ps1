...
function Find-CTlsCertificate
{
    ...
    * has 'Server Authentication' in its enhanced key usage list or has no enhanced key usage metadata.
    ...
    Additionally, you can use the `-Trusted` switch to only return trusted certificates, i.e. certificates whose issuing
    certificate authorities in its cert chain are installed in the local machine or current user's [trusted certificate
    stores](https://docs.microsoft.com/en-us/dotnet/standard/security/cross-platform-cryptography#x509store).
    ...
    Enable debug messages to see detailed information about why a certificate is or isn't being found. You'll see
    messages for each selection criteria and if a criterium isn't met, you'll see a `!` flag. For example, this debug
    output from `Find-CTlsCertificate -HostName 'example.com' -Trusted`:
    ...
    matched the necessary criteria for a TLS certificate.

    If a certificate isn't found and debug messages are off, the search log is written to the verbose stream instead of
    the debug stream.
    ...
    Demonstrates how to find a TLS certificate for the current computer using the computer's hostname and domain name as
    determined by the `[System.Net.NetworkInformation.IPGlobalProperties]` object returned by the
    ...
    Demonstrates how to find a valid *trusted* TLS certificate by using the `-Trusted` switch. Trusted certificates are
    issued by certificate authorities whose certificates (and all certificates in the certificate chain) are in the
    local machine or current user's [trusted certificate
    stores](https://docs.microsoft.com/en-us/dotnet/standard/security/cross-platform-cryptography#x509store).
    #>
    [CmdletBinding()]
